...
def stock_prediction_tool():
    ...
    if symbol:
        ...
        st.markdown(f"<h3 style='color: blue;'>Yesterday's Closing Price for {symbol}: <b>${yesterday_close:.2f}</b></h3>", unsafe_allow_html=True)
        st.markdown(f"<h3 style='color: green;'>Predicted Closing Price for Today: <b>${predicted_price:.2f}</b></h3>", unsafe_allow_html=True)
